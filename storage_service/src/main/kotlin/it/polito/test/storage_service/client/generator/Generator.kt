package it.polito.test.storage_service.client.generator
import org.slf4j.LoggerFactory
import java.util.random.RandomGenerator
import java.util.random.RandomGeneratorFactory
//import random generator factory
...
class Generator(filename: String){
    ...
    companion object {
        ...
        private var files: MutableMap<String, Long> = mutableMapOf()
        ...
        fun setFileMapProperty(numberOfFiles: Int, maxDimension: Long= MAX_BYTEARRAY_SIZE_FOUND, fixedMaxDimension: Boolean = false): MutableMap<String, Long>{
            ...
            if(fixedMaxDimension)
                for(i in 1..numberOfFiles) {
                    newMap[nameGenerator.nextLong().toString()] = maxDimensionToUse
                }
            else {
                for (i in 1..numberOfFiles)
                    newMap[nameGenerator.nextLong().toString()] = psFileMapGenerator.nextLong(MIN_FILE_SIZE, maxDimensionToUse)
            }
            files=newMap
            return newMap
        }
        fun setFileMapPropertyWithTalkingName(numberOfFiles: Int, maxDimension: Long= MAX_BYTEARRAY_SIZE_FOUND, fixedMaxDimension: Boolean = false): MutableMap<String, Long>{
            val maxDimensionToUse =  if(maxDimension< MIN_FILE_SIZE) MAX_BYTEARRAY_SIZE_FOUND else maxDimension
            if(maxDimension< MIN_FILE_SIZE)
                logger.info("Max dimension is too small. Set to default value: $MAX_BYTEARRAY_SIZE_FOUND")
            if(fixedMaxDimension)
